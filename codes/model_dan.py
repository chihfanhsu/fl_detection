...
def __gen_heatmap(shapes, IMAGE_SIZE=112):
...
def conn_layers(imgs, mean_s, s_current, fc_current):   
    ...
    # heatmap generation
    T_hmap = __gen_heatmap(T_pts, IMAGE_SIZE//2)
    ...
    T_fmap = tf.image.resize_images(fm, (IMAGE_SIZE//2, IMAGE_SIZE//2), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    
    return T_img, T_hmap, T_fmap, T_pts, r, t

def DAN_blk(imgs, s_mean, s_current, fc_current, train_phase, keeprate, name = 'DAN_blk'):
    with tf.variable_scope(name):
        T_img, T_hmap, T_fmap, T_pts, r, t = conn_layers(imgs, s_mean, s_current, fc_current)

        igt_input = tf.concat([T_img, T_hmap, T_fmap], axis=3)
    
        #igt_bn = batch_norm(igt_input, train_phase, name='igt_bn')
...
def inference (image, s_mean, keep_prob, train_phase, debug):
    ...
    imgs_112 = tf.image.resize_images(image, (IMAGE_SIZE//2, IMAGE_SIZE//2), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    imgs_gray = tf.reduce_mean(imgs_112, axis=3, keepdims=True)
    
    delta_s, fc1 = FF_NN(imgs_gray, train_phase, keep_prob)
    s1_out = tf.cast(s_mean,dtype=tf.float32) + delta_s

    # DAN block
    ds2, fc2 = DAN_blk(imgs_gray, s_mean, s1_out, fc1, train_phase, keep_prob, name = 'DAN_blk1')
...
#     ds7, fc7 = DAN_blk(imgs_in_2, s_mean, ds6, fc6, train_phase, keep_prob, name = 'DAN_blk6')
    s2_out, _ = DAN_blk(imgs_gray, s_mean, ds2, fc2, train_phase, keep_prob, name = 'DAN_blk6')
