...
if conf.tar_model == 'dist':
    import model_dist as model
else:
...
def KL_divergence_tesorlfow(labels, logits, epsilon=1e-07):
    #KL_d = tf.reduce_sum(p*safe_ln_tf(p/(q+epsilon)))
    # minimize KL divergence is equivalent to minimize croess entropy
    KL_d = -tf.reduce_sum(labels*tf.log(logits + epsilon), axis=[1,2,3])
    return KL_d
...
def gen_gaumap(batch_landmarks):
    dist_map=[] 
    ...
    # np.asarray(dist_map) spend 0.2s
...
with graph.as_default() as g:
    ...
    annotation = tf.placeholder(tf.float32, shape=[None, IMAGE_SIZE, IMAGE_SIZE, 68], name="annotation")
    
    # for generating gaussian map
    train_phase = tf.placeholder(tf.bool, name='phase_train')
    
    # model structure
    logit = model.inference(image, keep_probability, train_phase, conf.debug) #[?,224,224,68]
    
    # reshape output
    in_shape = logit.get_shape()
    tmap = tf.transpose(logit, perm=[0,3,1,2])  #[?,68,224,224]

    tmap_reshape = tf.reshape(tmap, [-1, in_shape[3].value, in_shape[1].value*in_shape[2].value]) #[?,68,224x224]
    
    tmap_softmax = tf.nn.softmax(tmap_reshape)
    tmap_softmax_reshape = tf.reshape(tmap_softmax, [-1, in_shape[3].value, in_shape[1].value, in_shape[2].value])
    pred_annotation = tf.transpose(tmap_softmax_reshape, perm=[0,2,3,1])

    print("pred_annotation", pred_annotation)
    
    loss = tf.reduce_mean(KL_divergence_tesorlfow(annotation, pred_annotation))
    ...
    while True:
        ...
        if itr % 1000 == 0:
            ...
            for i in tqdm.trange(int((db_helen['testset']['pts'].shape[0])/conf.batch_size)):
                ...
                batch_ymap_valid = gen_gaumap(batch_ys_valid)
