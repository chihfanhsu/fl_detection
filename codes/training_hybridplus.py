...
if conf.tar_model == 'hybridplus':
    import model_hybridplus as model
else:
...
with graph.as_default() as g:
    ...
    annotation_68 = tf.placeholder(tf.int32, shape=[None, IMAGE_SIZE, IMAGE_SIZE, 68], name="annotation_map")
    train_phase = tf.placeholder(tf.bool, name='phase_train')

    # model structure
    pred_annotation, logits, heat_map, pred_pts = model.inference(image, keep_probability, train_phase, conf.debug)
    
    bg, logits_68 = tf.split(logits,[1,68],axis=3)
    
    in_shape = logits_68.get_shape()
    tmap = tf.transpose(logits_68, perm=[0,3,1,2])
    #print("tmap",tmap)
    tmap_reshape = tf.reshape(tmap, [-1, in_shape[3].value, in_shape[1].value*in_shape[2].value])
    #print("tmap_reshape",tmap_reshape)
    
    in_shape_lab = annotation_68.get_shape()
    labmap = tf.transpose(annotation_68, perm=[0,3,1,2])
    #print("tmap",tmap)
    labmap_reshape = tf.reshape(labmap, [-1, in_shape_lab[3].value, in_shape_lab[1].value*in_shape_lab[2].value])
    #print("tmap_reshape",tmap_reshape)
    ...
    # distribution loss
    kl_losses = tf.reduce_mean(
        tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=labmap_reshape,
                                                                 logits=tmap_reshape), axis =1)
    )

    tot_loss = heatmap_loss + 0.25*reg_losses + 0.01*kl_losses
...
if conf.training == True:
    ...
    while True:
        ...
        if itr % 1000 == 0:
            ...
            if (calc_v_loss < max_validloss) or (calc_l2_loss < max_validL2loss):
                ...
                if (calc_l2_loss < max_validL2loss):
                    max_validL2loss = calc_l2_loss
